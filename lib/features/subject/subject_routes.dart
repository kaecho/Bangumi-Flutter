...
final List<GoRoute> subjectRoutes = [
  ...
  GoRoute(
    path: '/subject/:id',
    builder: (_, state) =>
        SubjectScreen(id: int.tryParse(state.pathParameters['id'] ?? '') ?? 0),
  ),
  // 条目信息
  GoRoute(
  ...
  GoRoute(
    ...
    builder: (_, state) =>
        EpisodesScreen(id: int.tryParse(state.pathParameters['id'] ?? '') ?? 0),
  ),
  // 章节吐槽箱
  GoRoute(
  ...
  GoRoute(
    ...
    builder: (_, state) =>
        PersonsScreen(id: int.tryParse(state.pathParameters['id'] ?? '') ?? 0),
  ),
  // 评分分布
  GoRoute(
    path: '/subject/:id/rating',
    builder: (_, state) =>
        RatingScreen(id: int.tryParse(state.pathParameters['id'] ?? '') ?? 0),
  ),
  // 番剧截屏预览
  GoRoute(
    path: '/subject/:id/preview',
    builder: (_, state) =>
        PreviewScreen(id: int.tryParse(state.pathParameters['id'] ?? '') ?? 0),
  ),
  // 条目标签
  GoRoute(
    path: '/subject/:id/tag',
  ...
  GoRoute(
    ...
    builder: (_, state) =>
        VoicesScreen(id: int.tryParse(state.pathParameters['id'] ?? '') ?? 0),
  ),
  // 关联条目
  GoRoute(
    path: '/subject/:id/link',
    builder: (_, state) =>
        LinkScreen(id: int.tryParse(state.pathParameters['id'] ?? '') ?? 0),
  ),
  // 包含该条目的目录
  GoRoute(
    path: '/subject/:id/catalogs',
    builder: (_, state) =>
        CatalogsScreen(id: int.tryParse(state.pathParameters['id'] ?? '') ?? 0),
  ),
  // 条目讨论版
  GoRoute(
    path: '/subject/:id/board',
    builder: (_, state) => SubjectBoardScreen(
      id: int.tryParse(state.pathParameters['id'] ?? '') ?? 0,
    ),
  ),
  // 条目概览 (书籍/音乐)
  GoRoute(
    ...
    builder: (_, state) =>
        OverviewScreen(id: int.tryParse(state.pathParameters['id'] ?? '') ?? 0),
  ),
  // 维基编辑历史
  GoRoute(
    path: '/subject/:id/wiki',
    builder: (_, state) =>
        WikiScreen(id: int.tryParse(state.pathParameters['id'] ?? '') ?? 0),
  ),
  // 条目吐槽箱
  GoRoute(
