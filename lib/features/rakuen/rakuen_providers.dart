...
class TopicDetailNotifier extends FamilyAsyncNotifier<TopicPageData, String> {
  ...
  Future<TopicPageData> _fetch(String topicId, int page) async {
    ...
    if (parsed.isEmpty) {
      ...
      try {
        final json = await client.get(apiTopic(topicId)) as Map<String, dynamic>;
  ...
  TopicPageData _fromTopicJson(Map<String, dynamic> json) {
    ...
    return TopicPageData(
      ...
      group: (group['title'] as String? ?? group['name'] as String? ?? '').toString(),
...
final groupForumProvider =
    AsyncNotifierProvider.family<GroupForumNotifier, RakuenListData<RakuenTopicItem>, String>(
  GroupForumNotifier.new,
);

class GroupForumNotifier extends FamilyAsyncNotifier<RakuenListData<RakuenTopicItem>, String> {
  int _maxPage = 1;
  ...
  Future<void> loadMore() async {
    ...
    try {
      ...
      state = AsyncData(RakuenListData(
...
final groupMembersProvider = FutureProvider.family<List<GroupMember>, String>((ref, name) async {
  ...
  try {
    ...
    return list
        .map((e) {
          final user = e is Map ? Map<String, dynamic>.from(e) : const <String, dynamic>{};
          final avatar = user['avatar'] as Map<String, dynamic>? ?? const {};
          return GroupMember(
            userId: (user['id'] as num?)?.toInt().toString() ?? '',
            userName: (user['nickname'] as String? ?? user['username'] as String? ?? '').toString(),
...
final boardTopicsProvider =
    AsyncNotifierProvider.family<BoardTopicsNotifier, RakuenListData<RakuenTopicItem>, String>(
  BoardTopicsNotifier.new,
);

class BoardTopicsNotifier extends FamilyAsyncNotifier<RakuenListData<RakuenTopicItem>, String> {
  @override
  ...
  Future<RakuenListData<RakuenTopicItem>> _fetch(String scope, int page) async {
    ...
    final items = core
        ...
        .toList();
    return RakuenListData(items: items, page: page, hasMore: items.length >= 30);
  }
...
final notifyProvider =
    AsyncNotifierProvider.family<NotifyNotifier, NotifyListData, int>(NotifyNotifier.new);
...
class NotifyNotifier extends FamilyAsyncNotifier<NotifyListData, int> {
  ...
  Future<NotifyListData> _fetch(int page) async {
    ...
    return NotifyListData(
      items: list.map((e) => Notify.fromJson(Map<String, dynamic>.from(e as Map))).toList(),
      page: page,
  ...
  Future<void> loadMore() async {
    ...
    try {
      ...
      state = AsyncData(NotifyListData(
        items: [...current.items, ...next.items],
        page: next.page,
        hasMore: next.hasMore,
      ));
...
class HistoryNotifier extends Notifier<List<HistoryItem>> {
  ...
  Future<void> add(HistoryItem item) async {
    ...
    state = next;
    await Cache.instance.put(_boxName, _key, next.map((e) => e.toJson()).toList());
  }
...
/// 我的主题
final mineTopicsProvider = FutureProvider.family<List<Topic>, String>((ref, uid) async {
  final client = ref.read(apiClientProvider);
...
/// 我的日志
final mineBlogsProvider = FutureProvider.family<List<Blog>, String>((ref, uid) async {
  final client = ref.read(apiClientProvider);
  final json = await client.get(apiUserBlogs(uid));
  final list = _jsonList(json, const ['list']);
  return list.map((e) => Blog.fromJson(Map<String, dynamic>.from(e as Map))).toList();
});

/// 我的动态 (回复/吐槽等)
final mineTimelineProvider = FutureProvider.family<List<TimelineItem>, String>((ref, uid) async {
  final client = ref.read(apiClientProvider);
  final json = await client.get(apiUserTimeline(uid, 30));
  if (json is List) {
    return json.map((e) => TimelineItem.fromJson(Map<String, dynamic>.from(e as Map))).toList();
  }
  return const [];
});
...
final rakuenSearchProvider =
    AsyncNotifierProvider.family<SearchNotifier, RakuenListData<RakuenTopicItem>, String>(
  SearchNotifier.new,
);

class SearchNotifier extends FamilyAsyncNotifier<RakuenListData<RakuenTopicItem>, String> {
  @override
  Future<RakuenListData<RakuenTopicItem>> build(String keyword) async {
    return _fetch(keyword, 1);
  }

  Future<RakuenListData<RakuenTopicItem>> _fetch(String keyword, int page) async {
    final client = ref.read(apiClientProvider);
...
final reviewsProvider =
    AsyncNotifierProvider.family<ReviewsNotifier, ReviewListData, int>(ReviewsNotifier.new);
...
class ReviewsNotifier extends FamilyAsyncNotifier<ReviewListData, int> {
  ...
  Future<ReviewListData> _fetch(int subjectId, int page) async {
    final client = ref.read(apiClientProvider);
    final json = await client.get(apiSubjectReviews(subjectId), query: {'page': page});
    final list = _jsonList(json, const ['reviews']);
    return ReviewListData(
      reviews: list.map((e) => Review.fromJson(Map<String, dynamic>.from(e as Map))).toList(),
      page: page,
      hasMore: list.length >= 30,
    );
  }
  ...
  Future<void> loadMore() async {
    ...
    try {
      final next = await _fetch(arg, current.page + 1);
      state = AsyncData(ReviewListData(
...
String _userAvatar(Map<String, dynamic> user) {
  final avatar = user['avatar'] as Map<String, dynamic>? ?? const {};
  return (avatar['large'] as String? ?? avatar['medium'] as String? ?? '').toString();
}
